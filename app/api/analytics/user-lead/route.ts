...
export async function POST(request: NextRequest) {
    try {
        ...
        const communicationRecords = await db
            .select()
            .from(user_lead_communications)
            .where(
                and(
                    eq(user_lead_communications.user_id, parseInt(userId)),
                    eq(user_lead_communications.type, type)
                )
            )
            .orderBy(desc(user_lead_communications.created_at))
            .execute();
